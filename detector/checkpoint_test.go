...
// testIdent attests path for crafted journals, or zero when the
// path is fictional (refusal-path tests, which fail before trust).
func testIdent(path string) FileIdentity {
	ident, _ := FileIdentityOf(path)
	return ident
}
